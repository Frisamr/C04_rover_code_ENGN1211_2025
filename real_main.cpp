...
unsigned long MILLIS_PER_CM = 200;

// approximately correct
unsigned long MILLIS_PER_DEGREE = 21;

// set so the rover will drive approx 0.5cm
unsigned long SHORT_STEP_TIME = MILLIS_PER_CM / 2;

// set so the rover will drive 3cm
unsigned long LONG_STEP_TIME = MILLIS_PER_CM * 3;
...
SonarSystem sonarSys = SonarSystem {
  constants::SONAR_SYS_PINS, // pins
  Servo(), // servo
  0, // current angle (this will overridden when init is called)
};
...
void loop() {
  ...
  //testConstantMotion(255, 255, 10 * MILLIS_PER_CM);
}
